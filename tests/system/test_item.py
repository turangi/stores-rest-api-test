...
class ItemTest(BaseTest):
    ...
    def test_get_item_not_found(self):
        with self.app() as client:
            with self.app_context():
                ...
                self.assertEqual(resp.status_code, 404) # instructor's video shows this as a 404....
    ...
    def test_created_duplicate_item(self):
        with self.app() as client:
            with self.app_context():
                ...
                self.assertDictEqual({'message': "An item with name 'test' already exists."}, # alt: 'An item with name \'test\' aready exists.'
                                     json.loads(resp.data))
    ...
    def test_put_update_item(self):
        with self.app() as client:
            with self.app_context():
                StoreModel('test').save_to_db()
                ItemModel('test', 5.99, 1).save_to_db() # create an initial item

                self.assertEqual(ItemModel.find_by_name('test').price, 5.99) # verify that the initially created item exists
